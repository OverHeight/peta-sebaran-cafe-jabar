...
# else:
#     try:
df = pd.read_csv("./data/spatial/coffee_shop_jawa_barat_final.csv")
#     except:
#         data = {
...
st.title("☕ Peta Sebaran Café Jawa Barat")
st.markdown("---")
...
with col_map:
    ...
    for _, row in df.iterrows():
        folium.Marker(
            ...
            popup=f"""
            <b>{row['nama']}</b><br><br>
            Wilayah: {row['wilayah']}<br>
            Alamat: {row.get("alamat_asli")}
            """,
            icon=folium.Icon(color='orange', icon='coffee', prefix='fa')
        ).add_to(cluster)
    st_folium(m, width='110%', height=450)
...
with col_pie1:
    ...
    chart_wilayah = alt.Chart(wilayah_counts).mark_arc(innerRadius=50).encode(
        ...
        color=alt.Color(field="Wilayah", type="nominal"),
...
with col_pie2:
    ...
    chart_sumber = alt.Chart(sumber_counts).mark_arc(innerRadius=50).encode(
        theta=alt.Theta(field="Jumlah", type="quantitative"),
        color=alt.Color(field="Sumber", type="nominal", legend=alt.Legend(orient="bottom")),
